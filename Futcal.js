...
// icon-color: deep-green; icon-glyph: null;
...
if (!fm.fileExists(fm.joinPath(offlinePath, userSettingsOffline))) {
  ...
  if (iCloudUsed)
    await fm.downloadFileFromiCloud(
      fm.joinPath(offlinePath, userSettingsOffline)
    );
  userSettings = JSON.parse(
    fm.readString(fm.joinPath(offlinePath, userSettingsOffline))
  );
}
...
const teamDataApiUrl = encodeURI(
  `${baseApiUrl}/teams?id=${userSettings.teamId}&tab=overview&type=team&timeZone=${userSettings.timeZone}`
);
...
// Set tap URLs
const teamTapUrl = encodeURI(
  `${baseApiUrl}/teams/${userSettings.teamId}/overview`
);
const teamMatchesTapUrl = encodeURI(
  `${baseApiUrl}/teams/${userSettings.teamId}/fixtures`
);
let leagueTableTapUrl;
if (teamData) {
  const leagueOverviewUrl = encodeURI(
    `${baseApiUrl}${teamData.tableData.tables[0].pageUrl}`
  );
...
async function createWidget() {
  ...
  widget.backgroundColor = Color.dynamic(
    new Color(userSettings.backgroundColor.light),
    new Color(userSettings.backgroundColor.dark)
  );
...
async function addWidgetMatches(globalStack) {
  ...
  const nextMatchIndex = teamMatches.findIndex(
    (obj) => obj.notStarted && !obj.status.cancelled
  );
  ...
  if (match != undefined) {
    ...
    if (matchDetails.header.status.started) {
      if (match.home.score == match.away.score) {
        ...
        (matchDetails.header.teams[0].score >
          matchDetails.header.teams[1].score &&
          match.home.id == teamData.details.id) ||
        (matchDetails.header.teams[0].score <
          matchDetails.header.teams[1].score &&
          match.away.id == teamData.details.id)
      ) {
    ...
    const competitionValue = shortenLeagueRound(
      matchDetails.content.matchFacts.infoBox.Tournament.text
    );
    const competitionNameValue = competitionValue[0];
    addFormattedText(
      matchInfoCompetitionStack,
    ...
    if (userSettings.showOnlyOpposition) {
      if (userSettings.teamId == match.home.id) {
        // It's a home game for my team
        addFormattedText(
          matchInfoTeamsStack,
          teamsAwayValue,
    ...
    if (!matchDetails.header.status.started) {
      if (matchDetails.header.status.cancelled) {
        ...
        const detailsCancellationValue = replaceText(
          matchDetails.header.status.reason.long
        );
        addFormattedText(
          matchInfoDetailsStack,
          detailsCancellationValue,
          Font.regularSystemFont(12),
          Color.gray(),
          null,
          false
        ...
        const detailsDateValue = formatDate(
          new Date(matchDetails.content.matchFacts.infoBox["Match Date"])
        );
        addFormattedText(
          matchInfoDetailsStack,
          detailsDateValue,
        ...
        const detailsTimeValue = formatTime(
          new Date(matchDetails.content.matchFacts.infoBox["Match Date"])
        );
        addFormattedText(
          matchInfoDetailsStack,
      ...
      if (
        ...
        !matchDetails.header.status.finished
      ) {
        if (userSettings.showLivetime) {
          const detailsPlayingTimeValue = `(${replaceText(
            matchDetails.header.status.liveTime.short
          )})`;
          addFormattedText(
            matchInfoDetailsStack,
            detailsPlayingTimeValue,
            Font.regularSystemFont(12),
            Color.gray(),
            null,
            false
        ...
        addFormattedText(
          ...
          Font.semiboldSystemFont(11),
          Color.dynamic(
            new Color(userSettings.liveColor.light),
            new Color(userSettings.liveColor.dark)
          ),
          null,
          false
        );
...
async function addWidgetTable(stack) {
  ...
  if (!leagueTable) {
    ...
    for (let i = 0; i < teamData.tableData.tables[0].tables.length; i += 1) {
      teamFound = teamData.tableData.tables[0].tables[i].table.findIndex(
        (obj) => obj.id == teamData.details.id
      );
      if (teamFound != -1) {
        tableIndex = i;
        break;
    ...
    leagueSubtitle = leagueSubtitle.startsWith("- ")
      ? leagueSubtitle.substring(2)
      : leagueSubtitle;
  }
  // Get team position in league
  const teamOnLeague =
    leagueTable[leagueTable.findIndex((obj) => obj.id == teamData.details.id)];
  ...
  addFormattedText(
    ...
    Font.semiboldSystemFont(11),
    Color.dynamic(
      new Color(userSettings.leagueTitleColor.light),
      new Color(userSettings.leagueTitleColor.dark)
    ),
    1,
    false
  );
  ...
  for (let i = 0; i < table.length; i += 1) {
    ...
    for (let j = 0; j < table[i].length; j += 1) {
      ...
      if (i == 0 && j == highlighted) {
        const highlightedPositionImage = getPositionHighlight(
          teamLeaguePosition.toString(),
          Color.dynamic(
            new Color(userSettings.highlightedPositionColor.light),
            new Color(userSettings.highlightedPositionColor.dark)
          )
        );
        cellDataStack.addImage(highlightedPositionImage);
      } else if (i == 1 && j > 0) {
        let teamBadgeUrl = encodeURI(
          `${baseApiUrl}/images/team/${table[i][j]}_xsmall`
        );
...
function getTable(leagueTable, teamLeaguePosition) {
  ...
  if (teamLeaguePosition == -1) {
    ...
    highlighted = -1;
    console.log(
      "League Table Error: Team not found in the selected league, showing top teams."
    );
  } else if (teamLeaguePosition <= teamsAbove) {
    // If team selected in first place show 5 top teams and highlight first row
    initial = 1;
    final =
      teamsToShow <= leagueTable.length ? teamsToShow : leagueTable.length;
    highlighted = teamLeaguePosition;
  } else if (teamLeaguePosition > leagueTable.length - teamsBelow) {
    // If team selected in first place show 5 top teams and highlight first row
    initial =
      leagueTable.length - teamsToShow >= 0
        ? leagueTable.length - teamsToShow + 1
        : 1;
...
async function getImage(url, cachedFileName) {
  ...
  try {
    ...
    try {
      if (iCloudUsed)
        await fm.downloadFileFromiCloud(
          fm.joinPath(offlinePath, cachedFileName)
        );
...
async function getData(url, cachedFileName) {
  ...
  try {
    ...
    try {
      ...
      data = JSON.parse(
        fm.readString(fm.joinPath(offlinePath, cachedFileName))
      );
    } catch (err) {
      console.log(`${err}`);
    }
...
function formatDate(date) {
  if (isToday(date)) {
    ...
    dateFormatter.dateFormat = userSettings.showDayOfWeek
      ? "EEE dd/MMM"
      : "dd/MMM";
